namespace Neo4j.AspNet.Identity.Core.Tests
{
    using System;
    ...
    public class Neo4jUserStoreTests
    {
        public class Constructor
        {
            ...
            public async Task UserLabelPassedInIsUsed()
            {
                ...
                await userStore.FindByEmailAsync("n.e@email.com", new CancellationToken());
        ...
        public class FindByEmailAsyncMethod
        {
            ...
            public async Task LooksForEmailIgnoringCase(string email, string expectedEmail)
            {
                ...
                await userStore.FindByEmailAsync(email, new CancellationToken());
        ...
        public class FindByNameAsyncMethod
        {
            ...
            public async Task ThrowsArgumentException_WhenUsernameIsNullOrWhitespace(string username)
            {
                ...
                var ex = await Record.ExceptionAsync(() => userStore.FindByNameAsync(username, new CancellationToken()));
            ...
            public async Task LooksForUsernameIgnoringCase(string username, string expectedUsername)
            {
                ...
                await userStore.FindByNameAsync(username, new CancellationToken());

                
                mockGraphClient.Verify(gc => gc.ExecuteGetCypherResultsAsync<FindUserResult<IdentityUser>>(It.Is<CypherQuery>(actual => actual.ContainsParameter("p0", expectedUsername))), Times.Once);
            }
